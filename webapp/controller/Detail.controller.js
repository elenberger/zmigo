...
sap.ui.define([
	...
	return BaseController.extend("ztelia.controller.Detail", {
		...
		/**
		 * Event handler (attached declaratively) for the list delete button. Deletes the selected item. 
		 * @function
		 * @public
		 */
		onDelete: function(oEvt) {
			...
			//sObjectHeader = this._oODataModel.getProperty(sPath + "/Maktx"),
			var sPath = oEvt.getParameter("listItem").getBindingContext().getPath();
			var sObjectHeader = "";
			var sQuestion = this._oResourceBundle.getText("deleteText", sObjectHeader),
		...
		_onObjectMatched: function(oEvent) {
			...
			this.getScenario(oParameter.SC,
				//Success
				function(oData) {
					if (oData.Db_key) {
						this.getModel().metadataLoaded().then(function() {
							//oParameter.db_key = oData.Db_key;
							var sObjectPath = this.getModel().createKey("ZI_MDOC", {
								db_key: oData.Db_key
							}); //"ZI_MDOC'"+ oData.Db_key + "')"; //   //oParameter);
			...
			// this._oODataModel.read("/ScenarioSet('SC1')", {
			// 	async: true,
			...
			// 			this.getModel().metadataLoaded().then(function() {
			// 				oParameter.db_key = oData.Db_key;
			// 				var sObjectPath = this.getModel().createKey("ZI_MDOC", oParameter);
			// 				this._bindView("/" + sObjectPath);
			// 			}.bind(this));
			// 		}
			// 	}.bind(this)
			...
			//	this.getModel().metadataLoaded().then(function() {
			//		var sObjectPath = this.getModel().createKey("DocItemSet", oParameter);
			//		this._bindView("/" + sObjectPath);
			//	}.bind(this));
		},
		...
		_postMDOC_I: function() {

			var that = this;
			var oObj = this.getView().getBindingContext().getObject();

			this._oODataModel.callFunction("/ZI_MDOCPost", // function import name
				{
		...
		_fnUpdateSuccess: function(oData) {
            ...
            MessageToast.show("Document posted", {duration: 5000});
